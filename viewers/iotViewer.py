...
def parsear_dispositivos(ruta_xml):
    ...
    for match in re.finditer(r"<DEVICE>.*?</DEVICE>", contenido, re.DOTALL):
        ...
        if not tipo_match or tipo_match.group(1).strip() not in TIPOS_SOPORTADOS:
            continue

        nombre_match = re.search(r'<NAME translate="true">([^<]*)</NAME>', bloque)
        nombre = nombre_match.group(1).strip() if nombre_match else "Dispositivo IoT"
...
def main():
    ...
    if not dispositivos:
        print("No se encontraron dispositivos IoT en el archivo XML.")
        return
